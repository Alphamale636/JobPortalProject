...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT `password` FROM adminprofiles WHERE username = ?";
    ...
    if (password_verify($oldPassword, $storedPassword)) {
        ...
        $updateQuery = "UPDATE adminprofiles SET password = ? WHERE username = ?";
        $updateStmt = mysqli_prepare($conn, $updateQuery);
        mysqli_stmt_bind_param($updateStmt, "ss", $hashedNewPassword, $username);
        mysqli_stmt_execute($updateStmt);


        header('Location: Profile.php?passwordupdated=1');
        exit();
    } else {
        header('Location: Passchange.php?incorrectoldpassword=1');
...
<html lang="en">
    ...
    <body>
        <form class="form" method="POST" enctype="multipart/form-data" id="myForm">
            ...
            <p class="signin"><a href="Profile.php">&nbsp;Back To Profile</a> </p>
        </form>
        <script>
...
// SweetAlert2
    if (window.location.search.indexOf('incorrectoldpassword') > -1) {
        ...
        setTimeout(function () {
            window.location.href = 'Passchange.php'; // Redirect to the final target page
