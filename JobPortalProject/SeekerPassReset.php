...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Check if the token exists in the jobseekerprofiles table
    $sqlJobseeker = "SELECT `username` FROM jobseekerprofiles WHERE reset_token = ?";
    $stmtJobseeker = $conn->prepare($sqlJobseeker);
    $stmtJobseeker->bind_param("s", $token);
    $stmtJobseeker->execute();
    $stmtJobseeker->store_result();

    // Check if the token exists in the adminprofiles table
    $sqlAdmin = "SELECT `username` FROM adminprofiles WHERE reset_token = ?";
    ...
    if ($stmtJobseeker->num_rows === 1 || $stmtAdmin->num_rows === 1) {
        ...
        if ($stmtJobseeker->num_rows === 1) {
            $stmtJobseeker->bind_result($username);
            $stmtJobseeker->fetch();
            $table = "jobseekerprofiles";
        } else {
            $stmtAdmin->bind_result($username);
            $stmtAdmin->fetch();
            $table = "adminprofiles";
        }
        ...
        $sqlUpdate = "UPDATE $table SET password = ?, reset_token = NULL WHERE username = ?";
        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bind_param("ss", $hashedPassword, $username);
        
        if ($stmtUpdate->execute()) {
            // Password updated successfully
            header('Location: Login.php?passwordreset=1');
        } else {
            // Error updating password
            header('Location: Login.php?reseterror=1');
        }
    } else {
        // Token is not valid
